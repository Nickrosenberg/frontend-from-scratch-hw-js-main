...
function startCountdown() {
    ...
    countdownDisplay.textContent = countdownValue; // Отображаем текущее значение отсчёта

    countdown = setInterval(() => {
        countdownValue -= 1; // Уменьшаем значение на 1

        if (countdownValue > 0) {
            countdownDisplay.textContent = countdownValue; // Обновляем отображение
        } else {
            countdownDisplay.textContent = '🚀'; // Показываем ракету
            clearInterval(countdown); // Останавливаем таймер
            countdown = null; // Сбрасываем таймер
        }
    }, 1000); // Каждую секунду
}
...
function cancelCountdown() {
    if (countdown) {
        ...
        countdownValue = 3; // Сбрасываем значение отсчета на 3
    }
}